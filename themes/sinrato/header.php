...
<head>
<?php $sinrato_opt = get_option( 'sinrato_opt' ); ?>
	...
	<div class="page-wrapper">
		<?php 
			if(isset($sinrato_opt['header_layout']) && $sinrato_opt['header_layout']!=''){
				$header_class = str_replace(' ', '-', strtolower($sinrato_opt['header_layout']));
			} else {
				$header_class = '';
			} 
					...
					if ( isset($sinrato_opt['header_layout']) && $sinrato_opt['header_layout'] != "") { 
						...
						if(count($jscomposer_templates) > 0) {
							foreach($jscomposer_templates as $jscomposer_template){
								if($jscomposer_template->post_title == $sinrato_opt['header_layout']){ ?>
						...
						if ( isset($sinrato_opt['header_mobile_layout']) && $sinrato_opt['header_mobile_layout'] != "") {
							if(count($jscomposer_templates) > 0) {
								foreach($jscomposer_templates as $jscomposer_template){
									if($jscomposer_template->post_title == $sinrato_opt['header_mobile_layout']){ ?>
						...
						if ( isset($sinrato_opt['sticky_header']) && $sinrato_opt['sticky_header']) {
							if(count($jscomposer_templates) > 0) {
								foreach($jscomposer_templates as $jscomposer_template){
									if($jscomposer_template->post_title == $sinrato_opt['header_sticky_layout']){ ?>
								...
								<div class="container">
									<div class="row">
										<div class="col-12 col-lg-5 col-xl-4">
											<?php if( isset($sinrato_opt['logo_main']['url']) && $sinrato_opt['logo_main']['url']!=''){ ?>
												<div class="logo"><a href="<?php echo esc_url( home_url( '/' ) ); ?>" title="<?php echo esc_attr( get_bloginfo( 'name', 'display' ) ); ?>" rel="home"><img src="<?php echo esc_url($sinrato_opt['logo_main']['url']); ?>" alt=" <?php echo esc_attr( get_bloginfo( 'name', 'display' ) ) ?> " /></a></div>
											<?php } else { ?>
								...
								<div class="container">
									...
									<?php if ( has_nav_menu( 'mobilemenu' ) ) : ?>
										<div class="visible-small mobile-menu"> 
											<div class="mbmenu-toggler"><?php echo esc_html($sinrato_opt['mobile_menu_label']);?><span class="mbmenu-icon"><i class="fa fa-bars"></i></span></div>
