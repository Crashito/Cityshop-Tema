<?php
function sinrato_logo_shortcode( $atts ) {
...
function sinrato_mainmenu_shortcode( $atts ) {
...
function sinrato_stickymenu_shortcode( $atts ) {
	$sinrato_opt = get_option( 'sinrato_opt' );
	$atts = shortcode_atts( array(
		'style'  	=> 'style1',
	), $atts, 'roadstickymenu' );
...
function sinrato_mobilemenu_shortcode( $atts ) {
	$sinrato_opt = get_option( 'sinrato_opt' );
	$atts = shortcode_atts( array(
		'style'  	=> 'style1',
	), $atts, 'roadmobilemenu' );
	...
	ob_start(); ?>
		<div class="visible-small mobile-menu <?php echo esc_attr($atts["style"]) ?>"> 
			<div class="mbmenu-toggler"><?php if(isset($sinrato_opt['mobile_menu_label'])) { echo esc_html($sinrato_opt['mobile_menu_label']);}?><span class="mbmenu-icon"><i class="fa fa-bars"></i></span></div>
...
function sinrato_roadcategoriesmenu_shortcode ( $atts ) {
	$sinrato_opt = get_option( 'sinrato_opt' );
	$atts = shortcode_atts( array(
		...
		<div class="categories-menu-wrapper <?php echo esc_attr($atts["style"]) ?>">
			<div class="categories-menu-inner">
				<div class="categories-menu visible-large <?php echo esc_attr($cat_menu_class); ?>">
					<div class="catemenu-toggler"><span><?php if(isset($sinrato_opt['categories_menu_label'])) { echo esc_html($sinrato_opt['categories_menu_label']); } else { esc_html_e('ALL CATEGORIES', 'sinrato'); } ?></span></div>
					<div class="catemenu">
						<div class="catemenu-inner">
							...
							<div class="morelesscate">
								<span class="morecate"><i class="fa fa-plus"></i><?php if ( isset($sinrato_opt['categories_more_label']) && $sinrato_opt['categories_more_label']!='' ) { echo esc_html($sinrato_opt['categories_more_label']); } else { esc_html_e('More Categories', 'sinrato'); } ?></span>
								<span class="lesscate"><i class="fa fa-minus"></i><?php if ( isset($sinrato_opt['categories_less_label']) && $sinrato_opt['categories_less_label']!='' ) { echo esc_html($sinrato_opt['categories_less_label']); } else { esc_html_e('Close Menu', 'sinrato'); } ?></span>
							</div>
						</div>
					</div>
				</div>
...
function sinrato_roadsocialicons_shortcode( $atts ) {
	$sinrato_opt = get_option( 'sinrato_opt' );
	$atts = shortcode_atts( array(
		'style'  => 'style1',
	...
	ob_start();
	if(isset($sinrato_opt['social_icons'])) {
		echo '<ul class="social-icons">';
		foreach($sinrato_opt['social_icons'] as $key=>$value ) {
			if($value!=''){
				if($key=='vimeo'){
...
function sinrato_roadminicart_shortcode( $atts ) {
	$atts = shortcode_atts( array(
		'style'  => 'style1',
	), $atts, 'roadminicart' );
	$html = '';
...
function sinrato_roadwishlist_shortcode( $atts ) {
	$atts = shortcode_atts( array(
		'style'  => 'style1',
	), $atts, 'roadwishlist' );
	$html = '';
			...
			<div class="header-wishlist <?php echo esc_attr($atts["style"]) ?>">
				<div class="header-wishlist-inner">
					<a href="<?php echo get_permalink(get_option('yith_wcwl_wishlist_page_id')); ?>" class="wishlist-link">
						...
						<span class="wishlist-text"><?php esc_html_e('Wishlist','sinrato') ?></span>
					</a>
				</div>
			</div>
		<?php } ?>
...
function sinrato_roadproductssearch_shortcode( $atts ) {
	$atts = shortcode_atts( array(
		'style'  => 'style1',
  		...
  		<div class="header-search">
  			<div class="search-without-dropdown <?php echo esc_attr($atts["style"]) ?>">
		  		<div class="categories-container">
		  			<div class="cate-toggler-wrapper"><div class="cate-toggler"><span class="cate-text"><?php esc_html_e('All Categories', 'sinrato');?></span></div></div>
...
function sinrato_roadproductssearchdropdown_shortcode( $atts ) {
	$atts = shortcode_atts( array(
		'style'  => 'style1',
...
function sinrato_brands_shortcode( $atts ) {
	global $sinrato_opt;
	$brand_index = 0;
	if(isset($sinrato_opt['brand_logos'])) {
		$brandfound = count($sinrato_opt['brand_logos']);
	}
	$atts = shortcode_atts( array(
		'rows'             => '1',
		'items_1500up'     => '5',
	...
	if(isset($sinrato_opt['brand_logos']) && $sinrato_opt['brand_logos'] && $sinrato_opt['brand_logos'][0]['url'] != '') {
		$html .= '<div class="brands-carousel roadthemes-slider '.$atts["navigation_style"].' '.$atts["style"].'" data-margin='.$margin.' data-1500up='.$atts["items_1500up"].' data-1200-1499='.$atts["items_1200_1499"].' data-992-1199='.$atts["items_992_1199"].' data-768-991='.$atts["items_768_991"].' data-640-767='.$atts["items_640_767"].' data-480-639='.$atts["items_480_639"].' data-0-479='.$atts["items_0_479"].' data-navigation='.$navigation.' data-pagination='.$pagination.' data-speed='.$atts["speed"].' data-loop='.$loop.' data-auto='.$auto.'>';
			foreach($sinrato_opt['brand_logos'] as $brand) {
				if(is_ssl()){
...
function sinrato_imageslider_shortcode( $atts ) {
	global $sinrato_opt;
	$image_slider_index = 0;
	if(isset($sinrato_opt['image_slider'])) {
		$image_slider_found = count($sinrato_opt['image_slider']);
	}
	$atts = shortcode_atts( array(
		'rows'             => '1',
		'items_1500up'     => '4',
	...
	if(isset($sinrato_opt['image_slider']) && $sinrato_opt['image_slider'] && $sinrato_opt['image_slider'][0]['url'] != '') {
		$html .= '<div class="image-slider roadthemes-slider '.$atts["navigation_style"].' '.$atts["style"].'" data-margin='.$margin.' data-1500up='.$atts["items_1500up"].' data-1200-1499='.$atts["items_1200_1499"].' data-992-1199='.$atts["items_992_1199"].' data-768-991='.$atts["items_768_991"].' data-640-767='.$atts["items_640_767"].' data-480-639='.$atts["items_480_639"].' data-0-479='.$atts["items_0_479"].' data-navigation='.$navigation.' data-pagination='.$pagination.' data-speed='.$atts["speed"].' data-loop='.$loop.' data-auto='.$auto.'>';
			foreach($sinrato_opt['image_slider'] as $image) {
				if(is_ssl()){
...
function sinrato_counter_shortcode( $atts ) {
	$atts = shortcode_atts( array(
		'image'  => '',
		'number' => '100',
		'text'   => 'text',
		), $atts, 'sinrato_counter' );
	$html = '';
...
function sinrato_popular_categories_shortcode( $atts ) {
	$atts = shortcode_atts( array(
		'category' => '',
		'image'    => ''
...
function sinrato_latestposts_shortcode( $atts ) {
	global $sinrato_opt;
	$post_index = 0;
	$atts = shortcode_atts( array(
		'posts_per_page'   => 10,
	...
	if($atts['image']=='wide'){
		$imagesize = 'sinrato-post-thumbwide';
	} else {
		$imagesize = 'sinrato-post-thumb';
	}
	if($atts['category']=='0'){
		$atts['category'] = '';
	}
					...
					$html.='<div class="post-info">';
						...
						$html.='<div class="post-meta">';
							$html.='<p class="post-author">';
								$html.= sprintf( wp_kses(__( '%s', 'sinrato' ), array('a'=>array('href'=>array()))), __('By', 'sinrato').' <a href="'.$author_url.'">'.$author_name.' </a>' );
							$html.='</p>';
							if(has_category('',$post->ID)) {
								$html.='<div class="post-category">';
									$html.= esc_html('in', 'sinrato');
									$html.= ' ';
						...
						$html.='<div class="post-excerpt">';
							$html.= Sinrato_Class::sinrato_excerpt_by_id($post, $length = $atts['length']);
						$html.='</div>';
						$html.='<a class="readmore" href="'.get_the_permalink($post->ID).'">'.'<span>' .esc_html($sinrato_opt['readmore_text']). '</span>'.'</a>';
					$html.='</div>';
				$html.='</div>';
			$html.='</div>';
...
function sinrato_testimonials_shortcode( $atts ) {
	$atts = shortcode_atts( array(
		'limit'            => '10',
		'orderby'          => 'menu_order',
		'order'            => 'DESC',
...
function sinrato_magnifier_options($att) {  
	...
	wp_enqueue_script('sinrato-magnifier', get_template_directory_uri() . '/js/product-magnifier-var.js');
	wp_localize_script('sinrato-magnifier', 'sinrato_magnifier_vars', array(
...
function sinrato_heading_title_shortcode( $atts ) {
	$atts = shortcode_atts( array(
		'heading_title'      => 'Title',
		'sub_heading_title'  => '',
		'heading_image'      => '',
	...
	$html.='<div class="heading-title '.$atts["style"].' ">';
		...
		if ($atts["heading_image"] != '') {
			$html.='<img src="'.wp_get_attachment_url( $atts['heading_image']).'" alt="'.esc_attr__( 'Heading title icon', 'sinrato' ).'" />';
		}
	$html.='</div>';
	return $html;
}
function sinrato_countdown_shortcode( $atts ) {
	$atts = shortcode_atts( array(
		'countdown_day'    => '1',
		'countdown_month'  => '1',
		'countdown_year'   => '2020',
...
function sinrato_newsletter_shortcode( $atts ) {
	$atts = shortcode_atts( array(
		'newsletter_title'       => '',
	...
	$html.='<div class="roadthemes-newsletter '.$atts["style"].'">';
		if ($atts["newsletter_title"] != '') {
			...
			if ($atts["newsletter_image"] != '') {
				$html.='<img src="'.wp_get_attachment_url( $atts['newsletter_image']).'" alt="'.esc_attr__( 'Newstter icon', 'sinrato' ).'" />';
			}
			$html.='</div>';
		}
		ob_start(); ?>
...
function sinrato_roadthemes_menu( $atts ) {
	$atts = shortcode_atts( array(
		'menu_image'       => '',
		'menu_title'       => 'Title',
		'nav_menu'         => '',
	...
	$html.='<div class="roadthemes-menu '.$atts["style"].'">';
		if ($atts["menu_image"] != '') {
			$html.='<div class="menu-image">';
				$html.='<img src="'.wp_get_attachment_url( $atts['menu_image']).'" alt="'.esc_attr__( 'Menu image', 'sinrato' ).'"/>';
			$html.='</div>';
		}
		if ($atts["menu_title"] != '') {
