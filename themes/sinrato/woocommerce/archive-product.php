...
$shoplayout = 'sidebar';
if(isset($sinrato_opt['shop_layout']) && $sinrato_opt['shop_layout']!=''){
	$shoplayout = $sinrato_opt['shop_layout'];
}
...
$sinrato_shop_main_extra_class = NULl;
if($shopsidebar=='left') {
	$sinrato_shop_main_extra_class = 'order-lg-last';
}
$main_column_class = NULL;
switch($shoplayout) {
	case 'fullwidth':
		Sinrato_Class::sinrato_shop_class('shop-fullwidth');
		$shopcolclass = 12;
		$shopsidebar = 'none';
		$productcols = 4;
		break;
	default:
		Sinrato_Class::sinrato_shop_class('shop-sidebar');
		$shopcolclass = 9;
		$productcols = 3;
...
<div class="main-container">
	...
	<div class="shop_content">
		<div class="container shop_content-inner">
			<div class="row">
				<div id="archive-product" class="page-content col-12 <?php echo 'col-lg-'.$shopcolclass; ?> <?php echo esc_attr($sinrato_viewmode);?> <?php echo ''.$main_column_class; ?> <?php echo esc_attr($sinrato_shop_main_extra_class);?>">
					...
					<div class="archive-border">
						<!-- shop banner -->
						<?php if( is_shop()){ ?>
							<?php if(isset($sinrato_opt['shop_banner']['url']) && ($sinrato_opt['shop_banner']['url'])!=''){ ?>
								<div class="shop-banner">
									<img src="<?php echo esc_url($sinrato_opt['shop_banner']['url']); ?>" alt="<?php esc_attr_e('Shop banner','sinrato') ?>" />
								</div>
							<?php } ?>
						<?php } ?>
						<?php if (is_product_category()) {
							if(isset($sinrato_opt['show_category_image']) && $sinrato_opt['show_category_image'] == true)  {
								global $wp_query;
						...
						<?php if ( woocommerce_products_will_display() ) { ?>
							<div class="toolbar">
								<div class="toolbar-inner">
									<div class="view-mode">
										<label><?php esc_html_e('View on', 'sinrato');?></label>
										<a href="#" class="grid <?php if($sinrato_viewmode=='grid-view'){ echo ' active';} ?>" title="<?php esc_attr_e( 'Grid', 'sinrato' ); ?>"><?php esc_html_e('Grid', 'sinrato');?></a>
										<a href="#" class="list <?php if($sinrato_viewmode=='list-view'){ echo ' active';} ?>" title="<?php esc_attr_e( 'List', 'sinrato' ); ?>"><?php esc_html_e('List', 'sinrato');?></a>
									</div>
									<?php
										/**
						...
						?>
						<?php if ( have_posts() ) : ?>	
							<div class="shop-products products <?php echo esc_attr($sinrato_viewmode);?> <?php echo esc_attr($shoplayout);?>">
