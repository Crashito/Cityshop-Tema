...
class RoadProducts {
	...
	public function road_vc_shortcodes() {
		...
		$order_by_values = array(
			'',
			__( 'Date', 'sinrato' )          => 'date',
			__( 'ID', 'sinrato' )            => 'ID',
			__( 'Author', 'sinrato' )        => 'author',
			__( 'Title', 'sinrato' )         => 'title',
			__( 'Modified', 'sinrato' )      => 'modified',
			__( 'Random', 'sinrato' )        => 'rand',
			__( 'Comment count', 'sinrato' ) => 'comment_count',
			__( 'Menu order', 'sinrato' )    => 'menu_order',
		);

		$order_way_values = array(
			'',
			__( 'Descending', 'sinrato' ) => 'DESC',
			__( 'Ascending', 'sinrato' )  => 'ASC',
		); 
		 
	//Sale products Roadthemes
	vc_map( array(
		'name'        => __( 'RT Sale products', 'sinrato' ),
		'base'        => 'sale_products_r',
		"icon"        => get_template_directory_uri() . "/images/road-icon.jpg",
		'category'    => __( 'Theme', 'sinrato' ),
		'description' => __( 'List all products on sale', 'sinrato' ),
		'params'      => array_merge(
			array(
				array(
					'type'        => 'textfield',
					'heading'     => esc_html__( 'Per page', 'sinrato' ),
					'value'       => 12,
					'save_always' => true,
					'param_name'  => 'per_page',
					'description' => esc_html__( 'How much items per page to show', 'sinrato' ),
				),
					array(
					'type'        => 'textfield',
					'heading'     => __( 'Columns', 'sinrato' ),
					'value'       => 4,
					'save_always' => true,
					'param_name'  => 'columns',
					'description' => __( 'How much columns grid', 'sinrato' ),
				),
				array(
					'type'        => 'dropdown',
					'heading'     => __( 'Order by', 'sinrato' ),
					'param_name'  => 'orderby',
					'value'       => $order_by_values,
					'save_always' => true,
					'description' => sprintf( __( 'Select how to sort retrieved products. More at %s.', 'sinrato' ), '<a href="http://codex.wordpress.org/Class_Reference/WP_Query#Order_.26_Orderby_Parameters" target="_blank">WordPress codex page</a>' ),
				),
				array(
					'type'        => 'dropdown',
					'heading'     => __( 'Sort order', 'sinrato' ),
					'param_name'  => 'order',
					'value'       => $order_way_values,
					'save_always' => true,
					'description' => sprintf( __( 'Designates the ascending or descending order. More at %s.', 'sinrato' ), '<a href="http://codex.wordpress.org/Class_Reference/WP_Query#Order_.26_Orderby_Parameters" target="_blank">WordPress codex page</a>' ),
				), 
	...
	vc_map( array(
		'name'        => __( 'RT Featured products', 'sinrato' ),
		'base'        => 'featured_products_r',
		"icon"        => get_template_directory_uri() . "/images/road-icon.jpg",
		'category'    => __( 'Theme', 'sinrato' ),
		'description' => __( 'Display products set as "featured"', 'sinrato' ),
		'params'      => array_merge(
		array(
			array(
				'type'        => 'textfield',
	...
	vc_map( array(
	'name'        => __( 'RT Recent products', 'sinrato' ),
	'base'        => 'recent_products_r',
	"icon"        => get_template_directory_uri() . "/images/road-icon.jpg",
	'category'    => __( 'Theme', 'sinrato' ),
	'description' => __( 'Lists recent products', 'sinrato' ),
	'params'      => array_merge( 
		array(
			array(
				'type'        => 'textfield',
	...
	vc_map( array(
	'name' => __( 'Top Rated Products', 'sinrato' ),
	'base' => 'top_rated_products_r',
	"icon"        => get_template_directory_uri() . "/images/road-icon.jpg",
	'category' => __( 'Theme', 'sinrato' ),
	'description' => __( 'List all top rated products', 'sinrato' ),
	'params'      => array_merge( 
		array(
			array(
				'type' => 'textfield',
				'heading' => __( 'Per page', 'sinrato' ),
				'value' => 12,
				'param_name' => 'per_page',
				'save_always' => true,
				'description' => __( 'How much items per page to show', 'sinrato' ),
			),
			array(
				'type' => 'textfield',
				'heading' => __( 'Columns', 'sinrato' ),
	...
	vc_map( array(
	'name'        => __( 'RT Bestselling products', 'sinrato' ),
	'base'        => 'bestselling_products_r',
	"icon"        => get_template_directory_uri() . "/images/road-icon.jpg",
	'category'    => __( 'Theme', 'sinrato' ),
	'description' => __( 'List best selling products on sale', 'sinrato' ),
	'params'      => array_merge( 
		array(
				array(
					'type'        => 'textfield',
	...
	//products Roadthemes
	vc_map( array(
	'name'        => __( 'RT Products', 'sinrato' ),
	'base'        => 'list_products_r',
	"icon"        => get_template_directory_uri() . "/images/road-icon.jpg",
	'category'    => __( 'Theme', 'sinrato' ),
	'description' => __( 'Show multiple products by ID or SKU.', 'sinrato' ),
	'params'      => array_merge(
		array(
			array(
				'type'        => 'textfield',
			...
			array(
				...
				'save_always' => true,
				'description' => __( 'Enter List of Products', 'sinrato' ),
			),
			array(
				'type'       => 'hidden',
				'param_name' => 'skus',
	...
	vc_map( array(
	'name'        => __( 'RT Product categories', 'sinrato' ),
	'base'        => 'product_categories_r',
	"icon"        => get_template_directory_uri() . "/images/road-icon.jpg",
	'category'    => __( 'Theme', 'sinrato' ),
	'description' => __( 'Display product categories loop', 'sinrato' ),
	'params'      => array_merge( 
			array(
				array(
					'type'        => 'textfield',
					'heading'     => __( 'Number', 'sinrato' ),
					'param_name'  => 'number',
					'description' => __( 'The `number` field is used to display the number of products.', 'sinrato' ),
				),
				array(
					'type'        => 'dropdown',
					'heading'     => __( 'Order by', 'sinrato' ),
				...
				array(
					...
					'description' => __( 'Hide empty', 'sinrato' ),
				),
				array(
					'type'       => 'autocomplete',
					'heading'    => __( 'Categories', 'sinrato' ),
					'param_name' => 'ids',
					'settings'   => array(
						'multiple' => true,
						'sortable' => true,
					...
					'save_always' => true,
					'description' => __( 'List of product categories', 'sinrato' ),
				), 
			),road_get_slider_setting()
		)
	)  );
		...
		vc_map( array(
		'name'        => __( 'RT Product category', 'sinrato' ),
		'base'        => 'product_category_r',
		"icon"        => get_template_directory_uri() . "/images/road-icon.jpg",
		'category'    => __( 'Theme', 'sinrato' ),
		'description' => __( 'Show multiple products in a category', 'sinrato' ),
		'params'      => array_merge(
			array(
				array(
					'type'        => 'textfield',
				...
				array(
					'type'        => 'dropdown',
					'heading'     => __( 'Category', 'sinrato' ),
					'value'       => $product_categories_dropdown,
					'param_name'  => 'category',
					'save_always' => true,
					'description' => __( 'Product category list', 'sinrato' ),
				),
			),road_get_slider_setting()
		)
	)  );
		...
		if ( ! empty( $query ) ) {
			...
			if ( is_object( $product_object ) ) {
				...
				if ( ! empty( $product_sku ) ) {
					$product_sku_display = ' - ' . __( 'Sku', 'sinrato' ) . ': ' . $product_sku;
				}

				$product_title_display = '';
				if ( ! empty( $product_title ) ) {
					$product_title_display = ' - ' . __( 'Title', 'sinrato' ) . ': ' . $product_title;
				}

				$product_id_display = __( 'Id', 'sinrato' ) . ': ' . $product_id;
	...
	public function productIdAutocompleteSuggester( $query ) {
		...
		if ( is_array( $post_meta_infos ) && ! empty( $post_meta_infos ) ) {
			foreach ( $post_meta_infos as $value ) {
				$data = array();
				$data['value'] = $value['id'];
				$data['label'] = __( 'Id', 'sinrato' ) . ': ' . $value['id'] . ( ( strlen( $value['title'] ) > 0 ) ? ' - ' . __( 'Title', 'sinrato' ) . ': ' . $value['title'] : '' ) . ( ( strlen( $value['sku'] ) > 0 ) ? ' - ' . __( 'Sku', 'sinrato' ) . ': ' . $value['sku'] : '' );
				$results[] = $data;
			}
		}
	...
	public function productCategoryCategoryAutocompleteSuggester( $query, $slug = false ) {
		...
		if ( is_array( $post_meta_infos ) && ! empty( $post_meta_infos ) ) {
			foreach ( $post_meta_infos as $value ) {
				...
				$data['label'] = __( 'Id', 'sinrato' ) . ': ' . $value['id'] . ( ( strlen( $value['name'] ) > 0 ) ? ' - ' . __( 'Name', 'sinrato' ) . ': ' . $value['name'] : '' ) . ( ( strlen( $value['slug'] ) > 0 ) ? ' - ' . __( 'Slug', 'sinrato' ) . ': ' . $value['slug'] : '' );
				$result[] = $data;
			}
		}
	...
	protected function productCategoryTermOutput( $term ) {
		...
		$term_slug_display = '';
		if ( ! empty( $term_slug ) ) {
			$term_slug_display = ' - ' . __( 'Sku', 'sinrato' ) . ': ' . $term_slug;
		}

		$term_title_display = '';
		if ( ! empty( $term_title ) ) {
			$term_title_display = ' - ' . __( 'Title', 'sinrato' ) . ': ' . $term_title;
		}

		$term_id_display = __( 'Id', 'sinrato' ) . ': ' . $term_id;
	...
	public static function getProductsFieldsList() {
		return array(
			__( 'SKU', 'sinrato' )               => 'sku',
			__( 'ID', 'sinrato' )                => 'id',
			__( 'Price', 'sinrato' )             => 'price',
			__( 'Regular Price', 'sinrato' )     => 'regular_price',
			__( 'Sale Price', 'sinrato' )        => 'sale_price',
			__( 'Price html', 'sinrato' )        => 'price_html',
			__( 'Reviews count', 'sinrato' )     => 'reviews_count',
			__( 'Short description', 'sinrato' ) => 'short_description',
			__( 'Dimensions', 'sinrato' )        => 'dimensions',
			__( 'Rating count', 'sinrato' )      => 'rating_count',
			__( 'Weight', 'sinrato' )            => 'weight',
			__( 'Is on sale', 'sinrato' )        => 'on_sale',
			__( 'Custom field', 'sinrato' )      => '_custom_',
		);
	}
	...
	public static function getOrderFieldsList() {
		return array(
			__( 'ID', 'sinrato' )                       => 'id',
			__( 'Order number', 'sinrato' )             => 'order_number',
			__( 'Currency', 'sinrato' )                 => 'order_currency',
			__( 'Total', 'sinrato' )                    => 'total',
			__( 'Status', 'sinrato' )                   => 'status',
			__( 'Payment method', 'sinrato' )           => 'payment_method',
			__( 'Billing address city', 'sinrato' )     => 'billing_address_city',
			__( 'Billing address country', 'sinrato' )  => 'billing_address_country',
			__( 'Shipping address city', 'sinrato' )    => 'shipping_address_city',
			__( 'Shipping address country', 'sinrato' ) => 'shipping_address_country',
			__( 'Customer Note', 'sinrato' )            => 'customer_note',
			__( 'Customer API', 'sinrato' )             => 'customer_api',
			__( 'Custom field', 'sinrato' )             => '_custom_',
		);
	}
